#!/usr/bin/env python3
"""
    pre_convnet.py - Convolutional Neural Network using pre-trained VGG-16
    ...
    if file != ".DS_Store" and file != ".DS_St":
        ...
        for img_file in sorted_aphanumeric(os.listdir(img_path)):
            if img_file != ".DS_Store":
                file_path = img_path + '/' + img_file
                image = cv2.imread(file_path)
                image = cv2.resize(image, (224, 224))

                image_rep = sess.run(model.imgrep, feed_dict={model.imgs:[image]})[0]
                seq_rep.append(image_rep.tolist())
